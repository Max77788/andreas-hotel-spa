...
function ChartTooltip({ active, payload, label }: { active?: boolean; payload?: { name?: string; value?: number; color?: string }[]; label?: string }) {
  ...
  return <div className="rounded-lg border border-neutral-200 bg-white px-3 py-2 text-xs shadow-lg"><p className="mb-1 font-bold text-neutral-700">{label}</p>{payload.map((item) => <p key={item.name} style={{ color: item.color }} className="font-semibold">{item.name}: {item.value}</p>)}</div>;
}
...
export default function AnalyticsPage() {
  ...
  return <div className="min-h-screen bg-neutral-100 p-8"><div className="max-w-5xl mx-auto">
    ...
    {!data ? <p className="text-xl font-bold">Loading...</p> : <><div className="grid md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">{[["Total events", data.total], ["Booking clicks", data.byEvent.booking_click || 0], ["Chat starts", data.byEvent.chat_started || 0], ["Page views", data.byEvent.page_view || 0], ["Contact forms", data.byEvent.contact_form_submission || 0], ["RFP submissions", data.byEvent.rfp_submission || 0]].map(([label, value]) => <div key={String(label)} className="bg-white border-[3px] border-neutral-300 p-6"><p className="text-sm uppercase tracking-widest text-neutral-500 font-bold">{label}</p><p className="text-4xl font-bold mt-2 text-neutral-900">{value}</p></div>)}</div>
    ...
    <div className="grid md:grid-cols-2 gap-5"><section className="bg-white border-[3px] border-neutral-300 p-6"><h2 className="text-2xl font-bold mb-1">Top pages</h2><p className="text-sm text-neutral-500 mb-4">Where anonymous activity is happening</p>{data.topPages.length ? <div className="h-[280px] w-full"><ResponsiveContainer width="100%" height="100%"><BarChart data={data.topPages.map(([page, count]) => ({ page: page.length > 24 ? `${page.slice(0, 21)}...` : page, count }))} layout="vertical" margin={{ top: 4, right: 12, left: 8, bottom: 4 }}><CartesianGrid strokeDasharray="3 3" stroke="#e7e5e4" horizontal={false} /><XAxis type="number" allowDecimals={false} tick={{ fontSize: 11, fill: "#78716c" }} axisLine={{ stroke: "#d6d3d1" }} tickLine={false} /><YAxis type="category" dataKey="page" width={100} tick={{ fontSize: 11, fill: "#57534e" }} axisLine={false} tickLine={false} /><Tooltip content={<ChartTooltip />} /><Bar dataKey="count" name="Views" fill={chartColors.pageViews} radius={[0, 4, 4, 0]} /></BarChart></ResponsiveContainer></div> : <p className="text-neutral-500">No events recorded yet.</p>}</section><section className="bg-white border-[3px] border-neutral-300 p-6"><h2 className="text-2xl font-bold mb-1">Event mix</h2><p className="text-sm text-neutral-500 mb-4">Conversions and engagement signals</p>{Object.keys(data.byEvent).length ? <div className="h-[280px] w-full"><ResponsiveContainer width="100%" height="100%"><BarChart data={Object.entries(data.byEvent).map(([event, count]) => ({ event: event.replaceAll("_", " "), count }))} margin={{ top: 4, right: 12, left: -18, bottom: 4 }}><CartesianGrid strokeDasharray="3 3" stroke="#e7e5e4" vertical={false} /><XAxis dataKey="event" angle={-28} textAnchor="end" height={58} interval={0} tick={{ fontSize: 10, fill: "#57534e" }} axisLine={{ stroke: "#d6d3d1" }} tickLine={false} /><YAxis allowDecimals={false} tick={{ fontSize: 11, fill: "#78716c" }} axisLine={false} tickLine={false} /><Tooltip content={<ChartTooltip />} /><Bar dataKey="count" name="Events" fill={chartColors.chatStarts} radius={[4, 4, 0, 0]} /></BarChart></ResponsiveContainer></div> : <p className="text-neutral-500">No events recorded yet.</p>}</section></div></>}
  </div></div>;
}
