...
export const metadata = {
  ...
  description: "Review our hotel policies including cancellation policy, check-in/check-out times, resort fees, and more for The Andreas Hotel & Spa in Palm Springs.",
};

export default async function PoliciesPage() {
  let policies = fallbackPolicies;
  try { const data = await getPolicies(); if (data.length) policies = data; } catch {}

  return (
    <main className="min-h-screen bg-[var(--hotel-cream)]">
      <Nav />
      <section className="relative py-28 md:py-36 overflow-hidden" style={{ backgroundImage:"url(/hotel-photos/exterior.jpg)", backgroundSize:"cover", backgroundPosition:"center" }}>
        ...
        <div className="relative max-w-7xl mx-auto px-6 md:px-10 text-center">
          ...
          <h1 className="font-display text-4xl md:text-5xl lg:text-6xl text-white tracking-wide">Hotel Policies</h1>
        </div>
      </section>
      ...
      <section className="py-20 md:py-28">
        <div className="max-w-3xl mx-auto px-6 md:px-10">
          <p className="font-body text-[var(--hotel-charcoal)] leading-relaxed mb-16">
            ...
            <a href="tel:8883275701" className="text-[var(--hotel-terracotta)] hover:underline">888-327-5701</a>{" "}or{" "}
            <a href="mailto:stay@andreashotel.com" className="text-[var(--hotel-terracotta)] hover:underline">stay@andreashotel.com</a>.
          </p>

          <div className="space-y-10 mb-20">
            {policies.map((policy) => (
              <div key={policy.id}>
                <h3 className="font-body text-[10px] tracking-[0.35em] uppercase text-[var(--hotel-terracotta)] mb-2">{policy.label}</h3>
                <p className="font-body text-[var(--hotel-charcoal)]/90 leading-relaxed">{policy.detail}</p>
              </div>
            ))}
          </div>

          <div className="p-8 md:p-12 border border-[var(--hotel-terracotta)]/30" style={{ background:"rgba(201,169,110,0.06)" }}>
            <h2 className="font-body text-[10px] tracking-[0.35em] uppercase text-[var(--hotel-terracotta)] mb-3">Cancellation Policy</h2>
            <p className="font-body text-[var(--hotel-charcoal)] leading-relaxed text-lg">
              ...
              you will be charged for half the stay or one night's accommodation — whichever is greater.
            </p>
          </div>
        </div>
      </section>
